...
function updateStatus(message=""){const player=session.unitById("P1"),meteor=state.grid.tileAt(state.meteorTarget.x,state.meteorTarget.y),basin=state.grid.tileAt(1,4);status.textContent=[message,`Player grid=(${player.gridX},${player.gridY})`,`Meteor H${meteor.elevation} / Water ${Number(meteor.waterDepth||0).toFixed(2)}`,`Basin H${basin.elevation} / Surface ${Number(waterSurfaceZ(basin)??0).toFixed(2)}`,`Camera=${state.cameraQuarterTurns*90}° | revision=${state.revision}`].filter(Boolean).join(" ｜ ")}
session.subscribe((next,event)=>{renderer.sync(next);updateStatus(event.type==="INIT"?"Hydrology / Tactical / Climate / Environment core loaded.":event.type)});

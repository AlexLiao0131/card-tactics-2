...
import { pathTo } from "./tactical-engine.js";
export class BattleSession{
 ...
 move({unitId,x,y}){const unit=this.unitById(unitId);if(!unit?.alive)return{ok:false,reason:"UNIT_NOT_AVAILABLE"};const path=pathTo(this.state.grid,this.state.units,unit,x,y);if(!path.length)return{ok:false,reason:"ILLEGAL_MOVE"};const from={x:unit.gridX,y:unit.gridY};unit.gridX=x;unit.gridY=y;this.emit({type:"UNIT_MOVED",unitId,from,to:{x,y},path:path.map(t=>({x:t.x,y:t.y,elevation:t.elevation}))});return{ok:true,path}}
 meteor({x,y,setElevation=1}){const tile=this.state.grid.tileAt(x,y);if(!tile)return{ok:false,reason:"NO_TILE"};const events=deformTerrain(this.state.grid,x,y,{setElevation,source:"METEOR"});this.emit({type:"HYDROLOGY_UPDATED",source:"METEOR",events});return{ok:true,events}}
 addWater({x,y,amount=.35}){const tile=this.state.grid.tileAt(x,y);if(!tile)return{ok:false,reason:"NO_TILE"};const events=[],before=waterDepth(tile);addWater(tile,amount,events);redistribute(this.state.grid,{source:"DEBUG_ADD_WATER",events});this.emit({type:"HYDROLOGY_UPDATED",source:"DEBUG_ADD_WATER",x,y,from:before,to:waterDepth(tile),events});return{ok:true,events}}

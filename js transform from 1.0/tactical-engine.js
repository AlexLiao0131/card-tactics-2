export const TacticalEngine=(()=>{
  ...
  function canOccupyTerrain(u,t){if(!t||!TERRAINS[t.terrain]?.passable)return false;if(isAquatic(u))return isLiveWater(t);return true}
  function traversalElevation(t,u=null){if(isLiveWater(t)||window.ClimateEngine?.isSolidIce?.(t)){const surface=window.HydrologyEngine?.waterSurfaceZ?.(t);if(surface!=null)return Number(surface);}return elevation(t)}
  ...
  function canTraverseElevation(fromTile,toTile,u=null){if(!fromTile||!toTile)return false;const traits=terrainTraits(u);if(traits.includes("MOUNTAIN_WALK")&&(isMountainTile(fromTile)||isMountainTile(toTile)))return true;const delta=traversalElevation(toTile,u)-traversalElevation(fromTile,u);return delta<=MAX_NORMAL_CLIMB&&delta>=-MAX_NORMAL_DROP;}
  function canActiveMove(m,us,u,x,y,{ignoreElevation=false}={}){const from=tile(m,u.x,u.y),to=tile(m,x,y);if(!to||!canOccupyTerrain(u,to)||isBlockedByObject(m,x,y)||occupied(us,x,y,u.id))return false;return ignoreElevation||canTraverseElevation(from,to,u);}
  function cost(u,t){
  ...
  function reachable(m,us,u){ensureFacing(u);let max=u.character.combat.move,b=new Map([[K(u.x,u.y),0]]),q=[{x:u.x,y:u.y,c:0}];while(q.length){q.sort((a,b)=>a.c-b.c);let n=q.shift(),from=tile(m,n.x,n.y);for(let [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){let x=n.x+dx,y=n.y+dy,t=tile(m,x,y);if(!t||!canOccupyTerrain(u,t)||isBlockedByObject(m,x,y)||occupied(us,x,y,u.id)||!canTraverseElevation(from,t,u))continue;let c=n.c+cost(u,t),k=K(x,y);if(c<=max&&(!b.has(k)||c<b.get(k))){b.set(k,c);q.push({x,y,c})}}}b.delete(K(u.x,u.y));return b}
  ...
  function hasLineOfSight(m,u,target,s,environmentState=null){if(!m||!u||!target)return false;if(s?.ignoreVision!==true&&!canSee(m,u,target,environmentState))return false;if(attackType(u,s)!=="SHOT"||s?.trajectory==="ARC")return true;const from=tile(m,u.x,u.y),to=tile(m,target.x,target.y);if(!from||!to)return false;const fromEye=elevation(from)+0.5,toEye=elevation(to)+0.5;return lineCells(u,target).every(p=>{const middle=tile(m,p.x,p.y);if(!middle)return false;const rayHeight=fromEye+(toEye-fromEye)*p.t;return elevation(middle)<rayHeight;});}
